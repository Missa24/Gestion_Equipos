...
function formatDate(dateStr: string | null | undefined) {
    if (!dateStr) return "—";
    return new Intl.DateTimeFormat("es-BO", { dateStyle: "short" }).format(new Date(dateStr));
}
...
export function getEquipoColumns(
    ...
    return [
        ...
        },
        {
            ...
            header: "N° Serie",
            cell: ({ row }) => (
                <span className="font-mono text-xs">{row.getValue("numero_serie")}</span>
            ),
        },
        {
            accessorKey: "estado",
        ...
        },
        {
            ...
            header: "Ubicación",
            cell: ({ row }) => (
                <p className="max-w-[180px] truncate text-sm" title={row.getValue("ubicacion")}>
