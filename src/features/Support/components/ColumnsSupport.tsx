        ...
        return (
          <span
            className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${
              prioridadColor[p] ?? "bg-gray-100 text-gray-700 border-gray-200"
            }`}
          >
            {p}
          </span>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1">
          ...
          {rol !== "USER" && (
            <DialogSupport text="Editar" mode="edit" row={row.original} />
          )}
        </div>
      ),
      enableHiding: false,
